...
Canonical civic facts live in `data/civic/records.json` + `sources.json`; the
producer map in `docs/data-pipeline.md` is authoritative. Files marked
`canonical` there are GENERATED by `bun run data:generate` — never hand-edit
them or `data/civic/` directly. The full agent runbook (commands, order,
research-run requirements, promotion/conflict rules) is the
"Agent refresh runbook" section of `docs/data-pipeline.md`.
