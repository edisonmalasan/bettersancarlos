 ...
 * --------------------------------------------------------
 *   1. Immediately render a self-hosted feed from data/news.json — generated
 *      by `bun run data:generate` from canonical news records (Facebook posts
 *      flow in via pipeline research runs, never by direct overwrite). This always
...
(function () {
  ...
  function renderLocalFeed(container, articles, pageUrl) {
    ...
    if (!items.length) {
      ...
      for (var i = 0; i < items.length; i++) {
        ...
        html += '<span class="fb-post-page-name">LGU San Carlos</span>';
  ...
  function buildPluginIframe(pageUrl) {
    ...
    iframe.title = 'Latest posts from the Official LGU San Carlos Facebook Page';
    iframe.src = src;
    iframe.width = '500';
    iframe.height = '700';
    iframe.scrolling = 'no';
