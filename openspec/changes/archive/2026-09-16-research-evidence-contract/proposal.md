...
- New contract — standard envelope + type-specific body: small YAML frontmatter (`schema: research.v2`, stable `id`, `title`, `category`, `research_type`, `verification_status`, `temporal_status`, `risk`, `researched_at`, `last_checked` (descriptive only, never a staleness input), `canonical_domains`, optional `jurisdiction` (country/province/locality override), optional `data_files`); common sections (Scope, Summary, type-shaped Findings, Verification & Uncertainty, Conflicts, Gaps, Research Attempts, Sources, Notes); no empty-boilerplate requirement (FORMAT.md pins only the per-type minima).
...
- Claim/item-level verification where confidence differs within a file (e.g. emergency contacts table with per-row Verification/Temporal/As-of/Sources); narrative prose needs no per-sentence IDs; prefer the same stable semantic ID where an item clearly maps to a canonical record (naming only — never auto-promotion).
- Local source register per document: IDs exactly `S1`, `S2`, … (no leading zeros); references only in `Sources`-column table cells (`S1` or `S1, S3`) and prose inline code spans, with bare prose mentions unparsed; closed source types `official|archived-official|government-dataset|authoritative-secondary|secondary|community|other` (meanings in FORMAT.md); IDs document-local, never canonical `sources.json` IDs.
...
- Jurisdiction safety: omitting `jurisdiction` means the San Carlos City, Pangasinan, PH default; an override carries non-empty `country`/`province`/`locality`, and a non-default locality must be named in Scope — keeping district/provincial/national context explicit against wrong-city confusion with no repetitive frontmatter.
