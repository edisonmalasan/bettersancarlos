...
- [ ] 1.1 Prune decorative eyebrow labels per design D1: remove uppercase kicker labels in `src/app/page.tsx` (3), `src/app/contact/page.tsx` (3), `src/components/layout/Footer.tsx` (2), `src/app/services/health/page.tsx` (4), and remaining decorative instances in other files; keep only data-field labels (budget page keeps ~3 of 12) — verify `rg -c -i "uppercase tracking" src` drops from 34 to ≤8 and each survivor is a data-field label
- [ ] 1.2 Sweep stray blues per design D2: `#003399`→`#275230` (homepage search hover), `#00184d`→`#275230` (InfoBar), `#0066cc`→`#0077be` (SearchAutocomplete), `#0066a0`→`#005a8f` (contact badge gradient) — verify `rg -i -c "003399|00184d|0066cc|0066a0" src` returns nothing
- [ ] 1.3 Update archived-change reference: the standing audit list now includes the four stray hexes (documented in D2); run full audit `rg -i "0032a0|003d82|002170|f77f00|0044cc|06a77d|003399|00184d|0066cc|0066a0" src` — verify zero matches
- [ ] 1.4 Run `./node_modules/.bin/tsc --noEmit` — verify zero type errors after cleanup
