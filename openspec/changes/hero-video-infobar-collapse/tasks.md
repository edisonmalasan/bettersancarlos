...
- [x] 5.5 InfoBar refinement: py-1 (was missing padding entirely - root cause of tall strip), collapse height 30->26, gap-5/4/3 compact balance - verify slim strip (AMENDED in pass 2: the `justify-center` part of this task was user-rejected — desktop right-alignment restored, see 5.8)
- [x] 5.6 Sync OpenSpec artifacts: design D1/D2/D3 amendments + D7 feedback record; spec deltas updated (footage-visibility scenario, chrome-aware geometry, harmonized transition requirement, compact-strip requirement) (re-synced in pass 2 per D8: right-aligned wording)
- [ ] 5.7 Automated checks re-run (tsc, build) and user browser re-approval of task 4.3
- [x] 5.8 Fix pass-2 InfoBar regressions: restore right-aligned desktop layout (`justify-end`, ≤1024px centered as before), keep slim ~26px strip, replace Motion with IntersectionObserver + CSS max-height/opacity/padding transition (collapsed height now exactly 0), remove unused `motion` dependency (First Load JS 140→104 kB), live-data effects preserved — verified via automated checks (tsc 0 errors, production build pass, openspec validate --strict pass) and real-browser CDP check: rate 1 USD = ₱62.67, temp 26°C, date/clock (Asia/Manila) + PHT populated after hydration, collapse on scroll, re-expand at top, no hydration errors in React (one pre-existing cross-origin script error from the dotlottie classic-script load exists on the deployed site too — out of scope, see design D8). User browser re-approval still pending (5.7/4.3 remain open). Pass-2 addendum: vertical ink centering fixed via leading-none + flex items-center on strip/container (ink gaps 8.3/8.2px, ~24.5px desktop / 23.6px mobile, single line) after user review of the deployed pre-branch build showed top-heavy content; non-ASCII literals (₱ ° •) restored after an encoding-corrupting shell edit; PWAManager dev guard added (no SW registration + unregister leftovers in development — stale precache was serving old markup on localhost).
