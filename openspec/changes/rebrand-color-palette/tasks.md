...
- [x] 4.2 Verify statistics page (43 gradients) renders correctly after sweep: bars, animated metric cards, CMCI tabs — verify tabs toggle and bars animate in dev server with no console errors
...
- [x] 5.3 Run production build `$env:NODE_ENV="production"; ./node_modules/.bin/next build` — verify build succeeds; revert `tsconfig.tsbuildinfo` if dirtied
- [x] 5.4 Visual verification in dev server: homepage, services, statistics, budget, government pages in light mode, dark mode, and high-contrast mode — verify hero gradients, buttons, badges, links, and charts all render in the new palette with no blue remnants and WCAG AA pairings per design D1
