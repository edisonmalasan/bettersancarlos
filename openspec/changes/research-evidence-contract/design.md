...
- [Risk] Strict source-reference parsing false-positives on prose (e.g. "see S1" vs bare "S1") → Mitigation: validate only structured contexts (tables with Sources columns, explicit `Sources:` lists) and keep prose checks to obvious patterns; FORMAT.md defines the exact syntax
...
- None that change specs, approach, or tasks. Deferred implementation details (exact marker syntax for README regions, strictness level of prose source-reference parsing, whether `research:report` earns its keep) are settled during Phase 1–2 with the validator as the executable answer.
