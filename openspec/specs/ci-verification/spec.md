...
Defines the repository's automated verification contract: GitHub Actions CI runs the project's real verification steps (typecheck, production build) with a pinned toolchain on every PR and push to `main`, Renovate keeps dependencies and workflow actions current, and `main` cannot receive unverified changes.
