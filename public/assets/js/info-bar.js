...
const CONFIG = {
  ...
  SAN CARLOS_LAT: 16.5167,
  SAN CARLOS_LON: 121.1833,
...
const WeatherService = {
  ...
  async fetchWeather() {
    try {
      const response = await fetch(
        `https://api.open-meteo.com/v1/forecast?latitude=${CONFIG.SAN CARLOS_LAT}&longitude=${CONFIG.SAN CARLOS_LON}&current_weather=true`
      );
      ...
      if (data.current_weather && typeof data.current_weather.temperature === 'number') {
        return {
          ...
          timestamp: Date.now(),
          location: {
            lat: CONFIG.SAN CARLOS_LAT,
            lon: CONFIG.SAN CARLOS_LON,
            name: 'San Carlos, Pangasinan',
          },
        };
      }
